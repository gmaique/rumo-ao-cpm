...
describe('data/questoes.json — banco de questões CPM-PR 6º ano', () => {
  let questoes;
  ...
  test('contém ao menos 30 questões', () => {
    const raw = readFileSync(join(dataDir, 'questoes.json'), 'utf8');
    questoes = JSON.parse(raw);
    assert.ok(questoes.length >= 30, `esperado >= 30 questões, encontrado ${questoes.length}`);
  });
